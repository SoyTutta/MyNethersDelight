...
public class PoachingFailureRegistry {
    ...
    public static Optional<PoachingFailureCase> findCaseFor(Mob mob, ItemStack weapon) {
        return CASES.stream().filter(failureCase -> failureCase.matches(mob, weapon)).findFirst();
    }

    public static void registerAll() {
        CASES.clear();
        register(new PoachingFailureCase(EntityType.SPIDER, EntityType.CAVE_SPIDER, SoundEvents.ZOMBIE_VILLAGER_CONVERTED,
                (mob, weapon) -> mob.level().random.nextFloat() < 0.4F, CommonEvent::transferDataAndMakeHostile));
        ...
        register(new PoachingFailureCase(EntityType.FROG, EntityType.WITCH, SoundEvents.WITCH_CELEBRATE,
                (mob, weapon) -> (mob.level().getBiome(mob.blockPosition()).is(Biomes.SWAMP)
                        && mob.level().random.nextFloat() < 0.3F)
                        || (weapon.is(CommonTags.Items.TOOLS_KNIVES) && mob.level().random.nextFloat() < 0.3F),
                CommonEvent::transferDataAndMakeHostile));
        ...
        register(new PoachingFailureCase(EntityType.ALLAY, EntityType.VEX, SoundEvents.EVOKER_PREPARE_SUMMON,
                (mob, weapon) -> true, CommonEvent::transferDataAndMakeHostile));
        ...
        register(new PoachingFailureCase(EntityType.HORSE, EntityType.ZOMBIE_HORSE, SoundEvents.ZOMBIE_VILLAGER_CONVERTED,
                (mob, weapon) -> mob.level().random.nextFloat() < 0.4F
                        || (((AbstractHorse) mob).isTamed() && mob.level().random.nextFloat() < 0.2F),
                (original, replacement) -> CommonEvent.transferFullHorseData((AbstractHorse) original, (AbstractHorse) replacement)));
        register(new PoachingFailureCase(EntityType.ZOMBIE_HORSE, EntityType.SKELETON_HORSE, SoundEvents.ZOMBIE_INFECT,
                (mob, weapon) -> mob.level().random.nextFloat() < 0.2F
                        || (((AbstractHorse) mob).isTamed() && mob.level().random.nextFloat() < 0.1F),
                (original, replacement) -> CommonEvent.transferFullHorseData((AbstractHorse) original, (AbstractHorse) replacement)));
        register(new PoachingFailureCase(EntityType.ZOMBIE, EntityType.SKELETON, SoundEvents.ZOMBIE_INFECT,
                (mob, weapon) -> !(mob instanceof ZombieVillager) && !mob.isBaby()
                        && mob.level().random.nextFloat() < 0.3F, CommonEvent::transferDataAndMakeHostile));
        register(new PoachingFailureCase(EntityType.VILLAGER, EntityType.ZOMBIE_VILLAGER, SoundEvents.ZOMBIE_VILLAGER_CONVERTED,
                (mob, weapon) -> true, (original, replacement) -> {
                    Villager villager = (Villager) original;
                    ZombieVillager zombieVillager = (ZombieVillager) replacement;
                    ...
                    CommonEvent.makeHostile(replacement, original.getLastHurtByMob());
                }));
        register(new PoachingFailureCase(EntityType.WOLF, EntityType.WOLF, SoundEvents.WOLF_GROWL,
                ...
                (original, replacement) -> {
                    ...
                    tag.remove("Health");
                    replacement.readAdditionalSaveData(tag);
                    CommonEvent.transferBasicMobData(original, replacement);
                    ((Wolf) replacement).setTame(false);
                    CommonEvent.makeHostile(replacement, original.getLastHurtByMob());
                }));
        register(new PoachingFailureCase(EntityType.PIGLIN, EntityType.ZOMBIFIED_PIGLIN,
                SoundEvents.PIGLIN_BRUTE_CONVERTED_TO_ZOMBIFIED, (mob, weapon) -> true,
                CommonEvent::transferDataAndMakeHostile));
        register(new PoachingFailureCase(EntityType.PIGLIN_BRUTE, EntityType.ZOMBIFIED_PIGLIN,
                SoundEvents.PIGLIN_BRUTE_CONVERTED_TO_ZOMBIFIED, (mob, weapon) -> true,
                CommonEvent::transferDataAndMakeHostile));
    }

    private static void register(PoachingFailureCase failureCase) {
